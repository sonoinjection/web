...
- `POST /api/admin/update-status` — validates the status transition (see §9), updates the row and writes a `status_change` log. Sends nothing.
...
Cron / scheduled tasks are not implemented and no longer planned for email. The pre-course reminder (old Email 4) was dropped on 2026-09-11 along with Emails 3, 5 and 6. `registrations.reminder_sent_at` is retained but now unused — nothing writes it. A cron is still worth adding for a different reason: a weekly keep-alive query so the Supabase free tier never pauses for inactivity again.
...
automatically or opt-in from the admin board. They no longer exist —
Email 1 is the only message a registrant receives automatically, and
everything after it is a human reply from `kayit@`. The admin board still
collects the "katılımcı haberdar edildi" confirmations, but they now
record that a person did the informing rather than gating a send.
...
create table registrations (
  ...
  reminder_sent_at    timestamptz                       -- LEGACY: nothing writes this since the reminder email was dropped (2026-09-11)
);
...
| From → To | Action label (TR) | Gating in dialog | Log entries written |
|---|---|---|---|
| `applied` → `paid` | **Ödeme Onayla** | "Ödemenin alındığını teyit ediyorum." checkbox required | `status_change` |
| `applied` → `cancelled` | **İptal Et** | "Katılımcı haberdar edildi." checkbox required | `status_change` |
| `paid` → `cancelled` | **İptal Et** | same as above | same as above |
| `paid` → `refunded` | **İade Et** | "Katılımcı haberdar edildi." AND "Banka tarafında tamamlandı." both required; refund amount and notes optional | `status_change` |
| `cancelled` → `applied` | **Yeniden Aktive Et** | reason input optional | `status_change` only |
| `refunded` → `paid` | **Yeniden Aktive Et** | reason input optional | `status_change` only |
...
Admin status changes write a single `status_change` entry. `email_sent` entries now come only from `api/register.js` (Email 1 and Email 2 on application) — `update-status.js` dispatches nothing.
